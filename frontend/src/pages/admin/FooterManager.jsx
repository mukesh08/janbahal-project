...
const FooterManager = () => {
  ...
  useEffect(() => {
    axios.get('/api/pages/ensure-footer')
      .then(({ data }) => {
        setPage(data);
  ...
  useEffect(() => {
    if (!gjsCss) return;
    let tag = document.getElementById('footer-mgr-styles');
    if (!tag) { tag = document.createElement('style'); tag.id = 'footer-mgr-styles'; document.head.appendChild(tag); }
    tag.innerHTML = gjsCss;
    return () => document.getElementById('footer-mgr-styles')?.remove();
  }, [gjsCss]);
        ...
        <div style={s.previewCard}>
          ...
          <div style={s.previewFrame}>
            {loading ? (
              ...
              <div style={s.placeholder}>No footer content yet — click Open Visual Editor to get started.</div>
            )}
          </div>
        </div>
